...
### 方案验收层（提出任何方案、设计或推荐时逐条适用）

> 最小判据集 = **性能 + 非平凡性 + 支撑域**，三者缺一不可。判据"全部 PASS"本身不构成方案成立的证据。

30. **解读由数值生成**：分析代码里的解读性文字 MUST 由计算值分支产生（`if 实测值 > 阈值: 说 A else: 说 B`），**禁止与代码一同预写**。预写的叙述不会被后续结果挑战——实测中同一次运行内复发三次：声称"补交互"而选出的点只有单一水平、按猜测的编号命名文献（4 篇错 3 篇）、打印"误差显著放大"而实测为 0.99×。这是系统性倾向，不是偶发疏忽。
31. **非平凡性**：方案的判据集 MUST 含一条"方案与现状/基线有实质区别"，与性能判据并列。实测中定价方案的性能类判据全部 PASS，而实际调价任务占比为 `0.000`——约束交叠使"什么都不做"成为唯一可行解。缺此判据无法区分真实方案与退化解。
32. **支撑域**：模型输出作为可操作结论前，MUST 检查预测点落在训练特征包络（如各维 `[P1,P99]`）内的比例达到**预先声明**的阈值；低于阈值则降级到支撑子集，并**显式拒绝**为其余部分给出结论。实测中新场景仅 `2.9%` 落在训练域内，直接外推得出"60 元即达 70% 完成率、预测完成率 0.983"的虚高结论；降级后支撑域内结果与原场景实测同量级。注意包络检查是逐维的，查不出联合分布偏移。
33. **阈值锁定**：上述判据的阈值 MUST 在运行**之前**写入文件并记录时间戳，运行后不得修改。确需修改时作废本次判据、记录原因并重跑。**看完结果再放宽阈值来凑通过，等于三条全部失效**，比不做检查更危险——它会产出"已严格验收"的假象。

### 前沿方法层（仅当引入了 `frontier-cards.md` 的卡片）
34. 每张前沿卡是否有**同数据、同划分、同预算**的经典法对照表，且在 `RESULTS.md` 有运行记录 ID？无对照表 = 炫技，减分。
35. 是否超配额（全文 ≤ `frontier_cards_quota`、同族 ≤1 张）？
36. 卡片 failure_modes 标注的必检项是否逐条落实（如 A2 的三分数据划分图、A5 的 resample 纳入率、A8 的样本 i.i.d. 来源）？
...
2. **禁止"预期结果"**：不得写下未真正计算出的数值、图表或表格。来不及算就**如实缩减结论范围**，不得编造。这条同样管**解释性文字里的数字**——正文里"两者相差 0.27%""典型约 300 个采样点"这类顺手写下的量，以及代码注释与 docstring 里的标定值。它们比结果数字更容易漏网：不进 R-id 台账、读起来像背景说明而不像结论，却会被读者当作事实、被后续代码当作依据。**凡文中出现的数，要么现算，要么标出处。**（本条来自实测：同一会话内三次踩中——论文里的 OLS 系数与年龄相关系数、docstring 里把 2.93% 写成 0.27%、把实测 519 个采样点写成"约 300 个"。每次都是先顺手写下、随后自己当真。）
...
8. **检查报警的正确响应不总是「让它变绿」**。台账缺失、映射为空、覆盖不足这类结构性报警，最省事的应对是生成一张看起来完整的表——**而内容编造的台账比没有台账更糟**：它把"这一步没做"变成"这一步已核验"，恰好制造出证据链完整的假象，且事后极难分辨。
   分界线是**这一列由谁确定**：路径、哈希、文件大小、时间戳、脚本归属这类机器能扫出来的，用 `seed_*` 脚本种进去，那是真实数据的搬运；claim 关联、观测结论、风险判断、通过与否这类需要判断的，**留空并保持 `PENDING`**，不填 `<占位符>`（占位符会被 `LEDGER_PLACEHOLDER_ROW` 判为未填，而空值老实表示"还没填"）。
   补不上的就**如实登记为未修项**，让 Gate 停在 `NEEDS_HUMAN`。一个诚实的 `NEEDS_HUMAN` 比一个编出来的 `PASS` 有用得多。（本条来自实测：新上线的结构检查一次扫出某题 5 个台账未建，其中证据映射可由 `seed_evidence_map.py` 从真实文件种出并通过核对，而 `CLAIM_LEDGER.csv`/`NATURE_QA.csv` 需要语义判断——后者被保留为缺口，没有自动生成。）
...
- [ ] 论文 ≤20MB、未压缩、**是单独一个文件**；支撑材料 ≤20MB
- [ ] 页码自摘要页起连续、位置正确（`verify_paper_contract.py` 的 `pagination_check` 已机检：页脚页码须逐页等于 PDF 页序。常见坑是模板在摘要后 `\setcounter{page}{1}`，摘要标 1–2、正文又从 1 重新计，全文两套页码）
- [ ] PDF 可正常打开、字体嵌入、图片清晰
- [ ] **匿名扫描**：全文 + PDF 元数据 + 代码注释 + 文件路径 + 图片 EXIF 无学校/姓名/赛区
- [ ] 支撑材料解压后 `run_all --all --expect-problems <论文问数>` 可跑通，且 `[DONE]` 行打印的问数 = 论文问数（只看「没报错」会漏掉「只跑了一问」：实测支撑包 `src/` 只有 `p1.py` 而附录写 `p1.py…p5.py`，9.6 秒即 `[DONE]`）
- [ ] 官方结果模板（若有）已从提交副本回读，业务键、非零单元和总量与内部计划逐项一致；合并单元格、公式、header/footer 等 T3 登记结构未被编辑库静默删除

### 取消资格红线（格式规范原文，逐条留实测证据）
- [ ] **源程序完整可运行**：附录代码 = 支撑包代码 = 清环境实跑通过（第五条：缺少必要源程序或程序不能运行，可能被取消评奖资格）
- [ ] **运行结果与论文一致**：清环境重跑关键数值逐项等于论文/图表/`RESULTS.md`，差异已追根因（第五条：运行结果与论文不符，可能被取消评奖资格）
- [ ] **支撑材料与论文相符**：同版本冻结、哈希一致、未混旧副本（第十一条：不相符可能被取消评奖资格）

### 合规
- [ ] `rules-2026.md` 第五节 checklist 全绿（含检索纪律、AI、匿名与相似度自查）
- [ ] **本赛区附加要求已核验并满足**（第八条允许赛区另提要求；T0 未登记时在此阻断补查）
- [ ] `SEARCH_LOG.md` 无禁入域名采用记录（参赛规则第 5 条把"浏览"本身列为严重违纪）
