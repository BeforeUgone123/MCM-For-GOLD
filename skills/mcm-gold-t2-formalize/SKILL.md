...
3. 来源分为 A 官方/标准/同行评议、B 机构报告/数据文档、C 博客/论坛/AI 线索。C 级永不作为论文论据。
4. 对关键事实、参数区间、方法适用条件和失效模式至少做独立来源交叉验证；每条采用事实立即写 S-id。
5. 先形成来源支持的候选路线矩阵，再建模；禁止模型写完后反向拼引用。
6. 把可引用文本拆成单一 C-id，提取对象、条件、方向、范围和因果强度；为每个 S-id 标 `strong|partial|background|limiting|metadata-only`。
7. 标题或元数据相关不等于支撑。采用来源至少核对摘要或正文；`metadata-only` 永不进入论文，限制/冲突来源必须保留。
8. 用户明确要求 Nature/CNS 时才收窄期刊范围；普通国赛按权威性与适题性选择来源，不把 CNS 当作质量同义词。
...
每个小问均有输入、变量/参数、数学对象、物理输出、验证和跨问接口；关键事实有 S-id 与支撑等级，metadata-only 未被采用；假设有检验计划；路线有备选和停止条件。live 模式路线/简化边界等待 H-002，关键事实/单位/口径进入 H-003。等待时返回 `NEEDS_HUMAN`，不得把候选路线写成最终路线。
